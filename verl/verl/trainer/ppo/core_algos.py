...
# branch rollout adv compute
from collections import defaultdict
from typing import Optional, Tuple
import numpy as np
import torch
@register_adv_est(AdvantageEstimator.GRPO_BRANCH)  # or simply: @register_adv_est("grpo")
def compute_grpo_rollout_sum_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    ...
    """
    GRPO advantage with rollout-level SUM aggregation.

    Definition:
    - Each branch has an outcome reward r_{p,r,b}
    - Rollout reward R_{p,r} = sum_b r_{p,r,b}
    - GRPO is applied across rollouts within the same prompt
    - All branches in the same rollout share the same advantage

    Args:
        token_level_rewards: (bs, T) token-level rewards
        response_mask: (bs, T) response mask
        index: (bs,) prompt index
        sub_index: (bs,) rollout index (unique per prompt)
        epsilon: numerical stability
        norm_adv_by_std_in_grpo: whether to divide by std (GRPO vs Dr.GRPO)
    ...
    """
    # ------------------------------------------------------------------
    # Step 1: branch-level outcome reward
    # ------------------------------------------------------------------
    branch_scores = token_level_rewards.sum(dim=-1)  # (bs,)
    bsz = branch_scores.shape[0]

    # ------------------------------------------------------------------
    # Step 2: rollout-level reward = sum of branch rewards
    # key = (prompt_id, rollout_id)
    # ------------------------------------------------------------------
    rollout2sum = defaultdict(list)
    for i in range(bsz):
        key = (index[i], sub_index[i])
        rollout2sum[key].append(branch_scores[i])

    rollout_reward = {
        k: torch.stack(v).sum()
        for k, v in rollout2sum.items()
    }

    # ------------------------------------------------------------------
    # Step 3: prompt-level GRPO statistics over rollouts
    # ------------------------------------------------------------------
    prompt2rollouts = defaultdict(list)
    for (p, r), val in rollout_reward.items():
        prompt2rollouts[p].append(val)

    prompt_mean = {}
    prompt_std = {}
    for p, vals in prompt2rollouts.items():
        vals = torch.stack(vals)
        prompt_mean[p] = vals.mean()
        if vals.numel() > 1:
            prompt_std[p] = vals.std()
        else:
            prompt_std[p] = torch.tensor(1.0, device=vals.device)

    # ------------------------------------------------------------------
    # Step 4: rollout-level advantage
    # ------------------------------------------------------------------
    rollout_adv = {}
    for (p, r), val in rollout_reward.items():
        if norm_adv_by_std_in_grpo:
            rollout_adv[(p, r)] = (val - prompt_mean[p]) / (prompt_std[p] + epsilon)
        else:
            rollout_adv[(p, r)] = val - prompt_mean[p]

    # ------------------------------------------------------------------
    # Step 5: assign rollout advantage to each branch
    # ------------------------------------------------------------------
    branch_adv = torch.zeros_like(branch_scores)
    for i in range(bsz):
        branch_adv[i] = rollout_adv[(index[i], sub_index[i])]

    # ------------------------------------------------------------------
    # Step 6: broadcast to token-level and apply mask
    # ------------------------------------------------------------------
    branch_adv = branch_adv.unsqueeze(-1) * response_mask

    # outcome-RL: advantage == return
    return branch_adv, branch_adv
