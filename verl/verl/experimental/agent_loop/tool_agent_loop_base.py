...
OUTPUT format:
<tool_call>
{"name": "summarize", "arguments": {"fold_commit_ids": "NONE", "merged_commit": ""}}
</tool_call>
"""
...
FOLD_POLICY_EN = """
Decide whether to fold previous commits to save context budget.
OPTIONS:
- "NONE": Keep all commits (default)
...
Examples:
- Safe: fold_commit_ids="NONE", merged_commit=""
- Low context: fold_commit_ids="ALL", merged_commit="[key points from session]"
- Selective: fold_commit_ids="c0001,c0002", merged_commit="[merged content]"
"""

# V6.5 [8k GOOD]
BUDGET_TEMPLATE_EN_BEFORE = """\
[Context Budget Status]
- Current prompt (with latest tool outputs incorporated): {current_ctx_len} tokens
- Usable limit: {usable_limit} tokens
- Remaining tokens for next turn: {remaining_budget} tokens ({remaining_pct:.1f}% of usable context)
{fold_policy}
RULE: 
- Tool call response already injected into context. Default to no compression. No need to account for future tool responses.
- Compress only when context budget requires it, and always preserve key information.
"""

# BUDGET_TEMPLATE_EN_BEFORE = """\
# [Context Budget Status]
# - Current prompt length (with latest tool outputs incorporated): {current_ctx_len} tokens
# - Maximum usable context limit: {usable_limit} tokens
# - Remaining tokens available for next turn: {remaining_budget} tokens ({remaining_pct:.1f}% of usable context)
# {fold_policy}
# RULE: 
# - Tool call responses have already been injected into the current context. Default to no compression unless necessary.
# - Do not estimate or reserve tokens for future tool responses. Only account for what is currently in context.
# - Compress selectively when context budget requires it, always preserving key information and traceability.
# """
...
class AgentData:
    def __init__(
        ...
        self.valid_trajectory: bool = True
...
class ToolAgentLoopBase(AgentLoopBase, ABC):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        cls.max_depth = getattr(multi_turn, "max_depth", 10)
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> List[AgentLoopOutput]:
        ...
        def merge_delta_into_user(prompt: list, delta: list) -> list:
            ...
            # prompt[1]["content"] += f'\n\n<begin commit block>\n{summary}\n<end commit block>\n\nRETRIEVE commits. ASSESS: Sufficient for answer? → <answer>. Missing facts? → SEARCH. No summary of commits.'

            # prompt[1]["content"] += f'''\n\n<begin commit block>\n{summary}\n<end commit block>\n\nRETRIEVE commits. ASSESS:\n- Sufficient for all answers? → <answer>answer1; answer2 ...</answer> (answers MUST correspond 1-to-1 with question order, separated by semicolons)\n- Missing facts? → <tool_call>{{"name": "search", "arguments": {{"query_list": ["specific query"]}}}}</tool_call>\n No summary of commits.'''

            prompt[1]["content"] += f'''\n\n<begin commit block>\n{summary}\n<end commit block>
            First, analyze in tags:<think></think>
            Then output exactly ONE of:s
            - IF sufficient: <answer>answer1; answer2; ...</answer>  # 1-to-1 with question order
            - ELSE: <tool_call>{{"name": "search", "arguments": {{"query_list": ["..."]}}}}</tool_call> '''
            # return prompt

            # prompt[1]["content"] += f'''\n\n<begin commit block>\n{summary}\n<end commit block>
            # First, analyze the commit block
            # Then output exactly ONE of:
            # - IF sufficient: <answer>answer</answer>
            # - ELSE: <tool_call>{{"name": "search", "arguments": {{"query_list": ["..."]}}}}</tool_call> '''
            return prompt
        ...
        async def dfs(prompt_messages, delta_messages, image_data, depth, branch_id, registry):
            ...
            if depth == self.max_depth - 1:
                prompt_messages_delta.append({
                    "role": "user",
                    "content": (
                        "[CONTEXT_LIMIT] You have reached the maximum number of context management operations. "
                        "You MUST provide the final answer immediately based on existing information. "
                        "NO tool calls are allowed."
                        "Respond directly with your final answer in the required format. example: <answer>answer1;answer2;...</answer>"
                    )
                })
                # prompt_messages_delta.append({
                #     "role": "user",
                ...
                #         "You MUST provide the final answer immediately based on existing information. "
                #         "NO tool calls are allowed."
                #         "Respond directly with your final answer in the required format. example: <answer>answer</answer>"
                #     )
                # })
