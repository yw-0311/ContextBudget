...
def _passages2string(retrieval_result):
    ...
    for idx, doc_item in enumerate(retrieval_result):
        ...
        # content = keep_first_n_words(doc_item["document"]["text"],n=512)
        title = content.split("\n")[0]
        text = "\n".join(content.split("\n")[1:])
...
#     for idx, doc_item in enumerate(retrieval_result):
#         content = doc_item["document"]["text"]
#         title = doc_item["document"].get("title", "No Title")
#         text = "\n".join(content.split("\n"))
#         format_reference += f"Doc {idx + 1} (Title: {title})\n{text}\n\n"
#     return format_reference.strip()
...
def perform_single_search_batch(
    ...
    api_response = None
    error_msg = None
    print(f"DEBUG Starting batch search for {len(query_list)} queries.")
    print(f"[DEBUG] topk {topk}")
