...
#             SERVER_IP = first_line.split(":")[0] if first_line else ""
...
def call_search_api(
    ...
    # T4 H20
    headers = {"Content-Type": "application/json", "XRL-Authorization": "Bearer t-1429c96fn9o18503"}
    payload = {
            "queries": query_list,
            "return_scores": 1,
            "topk": topk
    }
    data = {"appId":"48011", "bizCode": "dev", "config": {"requestTimeoutMs": 60000}, "request": payload}

    # if retrieval_service_url == "http://127.0.0.1:8000/retrieve":
    if SERVER_IP:
        print(f"[DEBUG]:SERVER_IP {SERVER_IP}")
        retrieval_service_url = f"http://{SERVER_IP}:8000/retrieve"
    if retrieval_service_url == "http://127.0.0.1:8000/retrieve":
        headers = {"Content-Type": "application/json", "Accept": "application/json"}
        payload = {"queries": query_list, "topk": topk, "return_scores": 1}
    ...
    if error_msg:
        ...
        try:
            if retrieval_service_url == "http://127.0.0.1:8000/retrieve":
                ...
                raw_results = api_response.get("data", {}).get("data", [])
            if raw_results:
                pretty_results = []
                total_results = 0
