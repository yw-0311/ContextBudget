...
from verl.tools.utils.search_r1_like_utils_bc import perform_single_search_batch
...
class SearchTool(BaseTool):
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs) -> tuple[ToolResponse, float, dict]:
        ...
        original_count = len(query_list_from_params)
        if original_count > MAX_QUERY_LIST_LENGTH:
            query_list_from_params = query_list_from_params[:MAX_QUERY_LIST_LENGTH]
            logger.warning(
                f"[SearchTool] Query list truncated from {original_count} to {MAX_QUERY_LIST_LENGTH} queries. "
